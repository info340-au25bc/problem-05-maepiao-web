...
/* I got the output "84" in the console. I think it is becuase variable types in
javascript is declared dynamically. Therefore, when the input is a number, the
it takes it as number and process 4 as the number for calculating, and when the
input is a string, it takes is as string and add the "4" as a string for the
output.*/
...
function compoundInterest(initialBalance, annualInterest, years){
  return initialBalance*Math.E**(annualInterest*years);
}
...
function isFlush(cards){
  let card1rank = cards[0].suit;
  for(let i = 0; i < cards.length; i++){
    if(cards[i].suit != card1rank){
      return false;
    }
  }
  return true;
